...
```r
devtools::install_github("`r desc::desc_get_field("Organisation")`/`r desc::desc_get_field("Package")`", dependencies = TRUE)
```
...
if (!is.null(dictionary) && is.data.frame(dictionary)) {
  ...
  if (package_base_url == "") {
    github_org <- desc::desc_get_field("Organisation")
    package_name <- desc::desc_get_field("Package")
    package_base_url <- paste0("https://github.com/", github_org, "/",
                               package_name, "/raw/main/")
  }
  extdata_path <- paste0(package_base_url, "inst/extdata/")

  # URL for reference documentation
  reference_url <- paste0("https://", tolower(github_org), ".github.io/",
  ...
  tryCatch({
    dictionary |>
      ...
      dplyr::rename(dataset_name = file_name) |>
      dplyr::mutate(
        Dataset = paste0("[", dataset_name, "](", reference_url, dataset_name, ".html)"),
        CSV = paste0("[Download CSV](", extdata_path, dataset_name, ".csv)")
      ) |>
      dplyr::select(Dataset, CSV) |>
      knitr::kable()
  }, error = function(e) {
...
Data are available as [CC-BY](https://github.com/`r desc::desc_get_field("Organisation")`/`r desc::desc_get_field("Package")`/blob/main/LICENSE.md).
