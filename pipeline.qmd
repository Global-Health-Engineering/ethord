...
```{r}
system("git submodule add https://github.com/Global-Health-Engineering/gendict.git")
system("git submodule init")
reticulate::py_install("./gendict", method = "pip", args = c("--editable", "."))
```

```{python}
import os
import json
from gendict.core import gendict
...
for f_path in r.dataframes:
  context, response, descriptions = gendict(config["HELMHOLTZ_API_KEY"],
                                                f_path, 
                                                max_unique_values=5, 
                                                model=0, 
                                                temperature=0, 
  ...
  if descriptions:
      core.upload_descriptions_from_json(f_path, descriptions, in_file="data-raw/dictionary.csv")
```
