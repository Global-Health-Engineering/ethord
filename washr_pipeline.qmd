...
# Read data --------------------------------------------------------------------
details_in <- readr::read_csv("data-raw/ord_portal.csv") |>
  janitor::clean_names()

# Tidy data --------------------------------------------------------------------
## Clean the raw data into a tidy format here
details <- details_in |>
  rename(ord_portal_url = url) |>
  select(-researchers) |> 
  mutate(id = row_number())
# Export Data ------------------------------------------------------------------
usethis::use_data(details, overwrite = TRUE)
fs::dir_create(here::here("inst", "extdata"))
readr::write_csv(details,
                 here::here("inst", "extdata", paste0("details", ".csv")))
```
...
Requirements: \* deb: libjq-dev (Debian, Ubuntu). \* rpm: jq-devel (Fedora, EPEL) \* csw: libjq_dev (Solaris) \* brew: jq (OSX)
